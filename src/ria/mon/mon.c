...
static const size_t MON_COMMANDS_COUNT =
    sizeof MON_COMMANDS / sizeof *MON_COMMANDS;
...
void mon_run_command(const char *cmd) { mon_enter(false, cmd, strlen(cmd)); }
...
void mon_task(void) {
  ...
  if (ram_active() || rom_active() || fil_active())
    return;
