  ...
  if (usb_hid_leds_dirty) {
    ...
    for (uint8_t dev_addr = 1; dev_addr <= CFG_TUH_DEVICE_MAX; dev_addr++)
      for (uint8_t idx = 0; idx < CFG_TUH_HID; idx++)
        if (tuh_hid_interface_protocol(dev_addr, idx) ==
...
int usb_status_response(char *buf, size_t buf_size, int state) {
  ...
  int count_nfc = ch340_count();
  snprintf(buf, buf_size, STR_STATUS_USB, usb_count_hid_kbd,
           usb_count_hid_kbd == 1 ? STR_KEYBOARD_SINGULAR : STR_KEYBOARD_PLURAL,
           usb_count_hid_mou,
           usb_count_hid_mou == 1 ? STR_MOUSE_SINGULAR : STR_MOUSE_PLURAL,
           count_gamepad,
           count_gamepad == 1 ? STR_GAMEPAD_SINGULAR : STR_GAMEPAD_PLURAL,
           count_storage,
           count_storage == 1 ? STR_STORAGE_SINGULAR : STR_STORAGE_PLURAL,
           count_nfc, count_nfc == 1 ? STR_NFC_SINGULAR : STR_NFC_PLURAL);
  return -1;
}
...
static inline int usb_idx_to_hid_slot(int idx) { return HID_USB_START + idx; }
...
// Invoked when device with address is un-mounted
void tuh_umount_cb(uint8_t dev_addr) {
  printf("USB: Device unmounted - addr=%d\n", dev_addr);
  ch340_umount(dev_addr);
}
...
void tuh_mount_cb(uint8_t dev_addr) {
  ...
  // Check for CH340 (0x1A86:0x7523)
  if (vid == 0x1A86 && pid == 0x7523) {
    printf("USB: Detected CH340 for PN532\n");
    ch340_mount(dev_addr, vid, pid);
    return;
  }
}

void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t idx, uint8_t const *desc_report,
                      uint16_t desc_len) {
  bool valid = false;
  ...
  if (pad_mount(usb_idx_to_hid_slot(idx), desc_report, desc_len, vendor_id,
                product_id)) {
    ++usb_count_hid_pad;
    valid = true;
  }
